...
| `invasive-scenario.service.ts` (estendido) | Adiciona `impactMechanisms` nomeados, `affectedResources`, `establishmentPlausibility` (numérico), `spreadPressure`, `uncertainties` e `mvpAssumptions`. Catálogo de **21 perfis de invasoras** (mamíferos, aves, peixes, anfíbios e invertebrados) com `taxonGroup`/notas curadas. |
