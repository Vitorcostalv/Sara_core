...
test("VoiceService persists user and assistant turns and marks llm.generate as success", async () => {
  ...
  assert.equal(result.audioReplyUrl, null, "audioReplyUrl deve ser null quando TTS nao configurado");
  assert.deepEqual(
...
test("VoiceService returns audioReplyUrl when TTS synthesizes successfully", async () => {
  const service = new VoiceService(
    () => ({ transcribe: () => "Qual a temperatura hoje?" }),
    {
      audioConverter: () => undefined,
      userProfileRepository: {
        ensureLocalProfile: () => Promise.resolve({ id: "local-user" }),
      ...
      createToolCallsRepository: () => ({
        create: (input) => Promise.resolve(createToolCall("tool-1", { conversationTurnId: input.conversationTurnId, status: input.status, outputPayload: input.outputPayload ?? null, durationMs: input.durationMs ?? null })),
        updateStatusById: () => Promise.resolve(null),
      }),
      llmGenerator: {
        generate: () =>
          Promise.resolve({
            provider: "gemini",
            model: "gemini-2.5-flash",
            answer: "Resposta do LLM para TTS.",
            dryRun: false,
            contextPreview: "",
            factsPreview: [],
            ecosystems: [],
            grounding: { userId: "local-user", profileIncluded: true, factCount: 0, ecosystemsUsed: [], warnings: [] },
          } satisfies LlmGenerateResult),
      },
      ttsService: {
        synthesize: (_text, id, _lang) => `voice/audio/${id}`,
      },
    }
  );
  ...
  assert.equal(result.audioReplyUrl, `voice/audio/${result.requestId}`);
});

test("VoiceService returns audioReplyUrl null when TTS is unavailable", async () => {
  const service = new VoiceService(
    () => ({ transcribe: () => "Qual a temperatura hoje?" }),
    {
      audioConverter: () => undefined,
      ...
      llmGenerator: {
        generate: () =>
          Promise.resolve({
            provider: "gemini",
            model: "gemini-2.5-flash",
            answer: "Resposta disponivel para usuario.",
            dryRun: false,
            contextPreview: "",
            factsPreview: [],
            ecosystems: [],
  ...
  assert.equal(result.audioReplyUrl, null);
  assert.ok(result.assistantText, "assistantText deve permanecer presente quando TTS retorna null");
});
