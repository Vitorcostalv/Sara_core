...
function getApiBaseUrl(): string {
  const configuredBaseUrl = useUiStore.getState().apiBaseUrl?.trim();
  return configuredBaseUrl && configuredBaseUrl.length > 0 ? configuredBaseUrl : defaultApiBaseUrl;
}
...
export const tasksApi = {
  list: (query: ListTasksQuery) =>
    request<TasksListResponse>(withQuery("/tasks", query as Record<string, string | number | boolean | null | undefined>)),
  getById: (id: string) => request<TaskResponse>(`/tasks/${id}`),
  create: (payload: CreateTaskRequest) => request<TaskResponse>("/tasks", { method: "POST", body: payload }),
  update: (id: string, payload: UpdateTaskRequest) =>
    request<TaskResponse>(`/tasks/${id}`, { method: "PATCH", body: payload }),
  complete: (id: string) => request<TaskResponse>(`/tasks/${id}/complete`, { method: "PATCH" }),
  remove: (id: string) => request<void>(`/tasks/${id}`, { method: "DELETE" })
};

export const factsApi = {
  list: (query: ListFactsQuery) =>
    request<FactsListResponse>(withQuery("/facts", query as Record<string, string | number | boolean | null | undefined>)),
  getById: (id: string) => request<FactResponse>(`/facts/${id}`),
  create: (payload: CreateFactRequest) => request<FactResponse>("/facts", { method: "POST", body: payload }),
...
export const conversationTurnsApi = {
  list: (query: ListConversationTurnsQuery) =>
    request<ConversationTurnsListResponse>(
      withQuery("/conversation-turns", query as Record<string, string | number | boolean | null | undefined>)
    ),
  getById: (id: string) => request<ConversationTurnResponse>(`/conversation-turns/${id}`),
  create: (payload: CreateConversationTurnRequest) =>
    request<ConversationTurnResponse>("/conversation-turns", { method: "POST", body: payload })
};

export const toolCallsApi = {
  list: (query: ListToolCallsQuery) =>
    request<ToolCallsListResponse>(
      withQuery("/tool-calls", query as Record<string, string | number | boolean | null | undefined>)
    ),
  ...
  updateStatus: (id: string, payload: UpdateToolCallStatusRequest) =>
    request<ToolCallResponse>(`/tool-calls/${id}/status`, { method: "PATCH", body: payload })
};

export const userProfileApi = {
  get: () => request<UserProfileResponse>("/user-profile"),
  update: (payload: UpdateUserProfileRequest) =>
    request<UserProfileResponse>("/user-profile", { method: "PATCH", body: payload })
};
